def GetColorRGB(val, scale=True):
  """Get RGB value given extened HTML color name or hex value
  ...
  # TODO: Check that code works for all possible names and hex values.
  for color in colors():
    if color[key].lower() == val.lower():
      l = list(color["rgb"].replace("RGB(", "").replace(")", "").split(","))
      if scale == True:
        return [int(s)/255 for s in l]
      else:
...
if __name__ == '__main__':
  assert [255, 0, 0] == GetColorRGB('red', scale=False)
  assert [1, 0, 0] == GetColorRGB('red', scale=True)
  assert [0, 128, 0] == GetColorRGB('green', scale=False)
  assert [0, 128/255, 0] == GetColorRGB('green', scale=True)
  assert [0, 0, 255] == GetColorRGB('blue', scale=False)
  assert [0, 0, 1] == GetColorRGB('blue', scale=True)
