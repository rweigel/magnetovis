...
_, _, xAxisSource = mvs.axis(time=time, val='X', lims=[-15,15], coord_sys=csys)
mvs.tube(xAxisSource, vary_radius='By Scalar', tube_radius=0.05)

_, _, yAxisSource = mvs.axis(time=time, val='Y', lims=[-15,15], coord_sys=csys)
mvs.tube(yAxisSource, vary_radius='By Scalar', tube_radius=0.05)

_, _, zAxisSource = mvs.axis(time=time, val='Z', lims=[-15,15], coord_sys=csys)
